...
def report_loss(losses, wandb_run, wandb_step, tracking=False, mapping=False):
    ...
    if tracking:
        ...
        frame_opt_loss_dict[
            'Per Iteration Current Frame Optimization/step'] = wandb_step
...
def eval_online(dataset,
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        if mapping_iters == 0 and not add_new_gaussians:
            psnr = calc_psnr(im * presence_sil_mask,
                             curr_data['im'] * presence_sil_mask).mean()
        else:
    ...
    # yapf: disable
    fig.suptitle(
        "Average PSNR: {:.2f}, Average Depth L1: {:.2f}".format(
            avg_psnr, avg_l1),
        y=1.05,
        fontsize=16)
    # yapf: enable
    plt.savefig(os.path.join(eval_online_dir, "online_metrics.png"),
                bbox_inches='tight')
    if wandb_run is not None:
...
def eval_rendering_speed(dataset, final_params, num_frames, eval_every=1):
    print("Evaluating Rendering Speed ...")
    # psnr_list = []
    # rmse_list = []
    # l1_list = []
    # lpips_list = []
...
def eval_nvs(dataset,
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        # Skip frames if not eval_every (indexing accounts for first training
        # frame)
        ...
        percent_holes = (
            ~valid_region_mask).sum() / valid_region_mask.numel() * 100
        ...
        if save_frames:
            ...
            depth_colormap = cv2.applyColorMap(
                (normalized_depth * 255).astype(np.uint8), cv2.COLORMAP_JET)
            cv2.imwrite(
                os.path.join(render_rgb_dir,
                             "splatam_{:04d}.png".format(test_time_idx)),
                cv2.cvtColor(viz_render_im * 255, cv2.COLOR_RGB2BGR))
            cv2.imwrite(
                os.path.join(render_depth_dir,
                             "splatam_{:04d}.png".format(test_time_idx)),
    ...
    axs[0, 0].plot(np.arange(len(psnr_list)), psnr_list)
    axs[0, 0].set_title("RGB PSNR")
    axs[0, 0].set_xlabel("Time Step")
    axs[0, 0].set_ylabel("PSNR")
    axs[0, 1].plot(np.arange(len(l1_list)), l1_list * 100)
    axs[0, 1].set_title("Depth L1")
    axs[0, 1].set_xlabel("Time Step")
    axs[0, 1].set_ylabel("L1 (cm)")
    # yapf: disable
