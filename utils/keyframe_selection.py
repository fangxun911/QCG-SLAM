...
def get_pointcloud(depth, intrinsics, w2c, sampled_indices):
    ...
    B = torch.zeros((1, 3), device=A.device).float()
...
def keyframe_selection_overlap(gt_depth,
                               w2c,
                               intrinsics,
                               keyframe_list,
                               k,
    ...
    for keyframeid, keyframe in enumerate(keyframe_list):
        ...
        edge = 20
        mask = (projected_pts[:, 0] < width - edge) * (
            projected_pts[:, 0] > edge) * (projected_pts[:, 1] < height -
                                           edge) * (projected_pts[:, 1] > edge)
    ...
    # Sort the keyframes based on the percentage of points that are inside the
    # image
