...
def eval(dataset, final_params, num_frames, eval_dir, sil_thres, 
         mapping_iters, add_new_gaussians, swanlab_run=None, swanlab_save_qual=False, eval_every=1, save_frames=False):
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        valid_depth_mask = torch.ones_like(curr_data['depth'], dtype=torch.bool)  # 为了公平评估，所有像素都要评估
        ...
        if swanlab_run is None:
            ...
            plot_rgbd_silhouette(color, depth, im, rastered_depth_viz, presence_sil_mask, diff_depth_l1,
                                 psnr, depth_l1, fig_title, plot_dir, 
                                 plot_name=plot_name, save_plot=True,
                                 swanlab_run=swanlab_run, swanlab_step=None, 
    ...
    try:
        ...
        # Calculate ATE RMSE
        ate_rmse = evaluate_ate_and_save(gt_w2c_list, latest_est_w2c_list, eval_dir)
        print("Final Average ATE RMSE: {:.2f} cm".format(ate_rmse*100))
        if swanlab_run is not None:
            swanlab_run.log({"Final Stats/Avg ATE RMSE": ate_rmse,
                        "Final Stats/step": 1})
    except:
        ate_rmse = 100.0
    ...
    if swanlab_run is not None:
        swanlab_run.log({"Final Stats/Average PSNR": avg_psnr, 
    ...
    # np.savetxt(os.path.join(eval_dir, "rmse.txt"), rmse_list) 这里rmse和l1一模一样，没必要保存
...
def report_loss(losses, swanlab_run, swanlab_step, tracking=False, mapping=False):
    # Update loss dict
    ...
    if tracking:
        ...
        frame_opt_loss_dict['Per Iteration Current Frame Optimization/step'] = swanlab_step
...
def report_progress(params, data, i, progress_bar, iter_time_idx, sil_thres, every_i=1, qual_every_i=1, 
                    tracking=False, mapping=False, swanlab_run=None, swanlab_step=None, swanlab_save_qual=False, online_time_idx=None,
                    global_logging=True):
    if i % every_i == 0 or i == 1:
        ...
        if not (tracking or mapping):
            progress_bar.set_postfix({f"Time-Step: {iter_time_idx} | PSNR: {psnr:.{7}} | Depth L1": f"{rmse:.{7}}"})
            ...
            progress_bar.set_postfix({f"Time-Step: {iter_time_idx} | Pose Error: {iter_pt_error.item():.{7}} | ATE RMSE": f"{ate_rmse.item():.{7}}"})
            progress_bar.update(every_i)
        elif mapping:
            progress_bar.set_postfix({f"Time-Step: {online_time_idx} | Frame {data['id']} | PSNR: {psnr:.{7}} | Depth L1": f"{rmse:.{7}}"})
            progress_bar.update(every_i)
        
        if swanlab_run is not None:
            swanlab_log = {f"{stage}/PSNR": psnr,
        ...
        if swanlab_save_qual and (i % qual_every_i == 0 or i == 1):
            ...
            plot_rgbd_silhouette(data['im'], data['depth'], im, rastered_depth, presence_sil_mask, diff_depth_l1,
                                 psnr, depth_l1, fig_title, swanlab_run=swanlab_run, swanlab_step=swanlab_step, 
...
def plot_rgbd_silhouette(color, depth, rastered_color, rastered_depth, presence_sil_mask, diff_depth_l1,
                         ...
                         save_plot=False, swanlab_run=None, swanlab_step=None, swanlab_title=None, diff_rgb=None):
    ...
    if swanlab_run is not None:
        if swanlab_step is None:
            ...
            swanlab_run.log({swanlab_title: swanlab.Image(fig)}, step=swanlab_step)
