...
loss_fn_alex = LearnedPerceptualImagePatchSimilarity(net_type='alex',
                                                     normalize=True).cuda()
...
def eval(dataset,
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        color, depth, quadtree, intrinsics, pose = dataset[time_idx]
        ...
        if not np.isin(time_idx, final_params['keyframe_time_indices']):
            continue

        # Get current frame Gaussians
        transformed_gaussians = transform_to_frame(final_params,
                                                   time_idx,
        ...
        rendervar = transformed_params2rendervar(final_params,
        ...
        # valid_depth_mask = (curr_data['depth'] > 0)  # 只取深度大于0的部分
        valid_depth_mask = torch.ones_like(curr_data['depth'],
                                           dtype=torch.bool)  # 为了公平评估，所有像素都要评估
        ...
        if mapping_iters == 0 and not add_new_gaussians:
            ...
            weighted_gt_im = curr_data[
                'im'] * presence_sil_mask * valid_depth_mask
        else:
        ...
        if mapping_iters == 0 and not add_new_gaussians:
            diff_depth_rmse = torch.sqrt(
                (((rastered_depth - curr_data['depth']) *
                  presence_sil_mask)**2))
            ...
            diff_depth_rmse = torch.sqrt(
                (((rastered_depth - curr_data['depth']))**2))
            diff_depth_rmse = diff_depth_rmse * valid_depth_mask
            rmse = diff_depth_rmse.sum() / valid_depth_mask.sum()
        ...
        if save_frames:
            ...
            viz_render_im = viz_render_im.detach().cpu().permute(1, 2,
                                                                 0).numpy()
            vmin = 0
            vmax = 6
            viz_render_depth = rastered_depth_viz[0].detach().cpu().numpy()
            normalized_depth = np.clip(
                (viz_render_depth - vmin) / (vmax - vmin), 0, 1)
            ...
            cv2.imwrite(
                os.path.join(render_depth_dir,
                             "gs_{:04d}.png".format(time_idx)), depth_colormap)
            ...
            normalized_depth = np.clip((viz_gt_depth - vmin) / (vmax - vmin), 0,
                                       1)
            depth_colormap = cv2.applyColorMap(
                (normalized_depth * 255).astype(np.uint8), cv2.COLORMAP_JET)
            cv2.imwrite(os.path.join(rgb_dir, "gt_{:04d}.png".format(time_idx)),
                        cv2.cvtColor(viz_gt_im * 255, cv2.COLOR_RGB2BGR))
            cv2.imwrite(
                os.path.join(depth_dir, "gt_{:04d}.png".format(time_idx)),
                depth_colormap)

        # Plot the Ground Truth and Rasterized RGB & Depth, along with
        # Silhouette
    ...
    try:
        ...
        for idx in range(1, num_frames):  # 因为位姿每帧都优化了，所以算全部帧
        ...
        # Calculate ATE RMSE
        ate_rmse = evaluate_ate_and_save(gt_w2c_list, latest_est_w2c_list,
                                         eval_dir)
        print("Final Average ATE RMSE: {:.2f} cm".format(ate_rmse * 100))
        if swanlab_run is not None:
            swanlab_run.log({
                "Final Stats/Avg ATE RMSE": ate_rmse,
                "Final Stats/step": 1
            })
    ...
    # Plot PSNR & Depth L1 & SSIM & LPIPS as line plots
    fig, axs = plt.subplots(2, 2, figsize=(12, 10))
    axs[0, 0].plot(final_params['keyframe_time_indices'], psnr_list)
    axs[0, 0].set_title("RGB PSNR")
    axs[0, 0].set_xlabel("Time Step")
    axs[0, 0].set_ylabel("PSNR")
    axs[0, 1].plot(final_params['keyframe_time_indices'], l1_list * 100)
    axs[0, 1].set_title("Depth L1")
    axs[0, 1].set_xlabel("Time Step")
    axs[0, 1].set_ylabel("L1 (cm)")
    axs[1, 0].plot(final_params['keyframe_time_indices'], ssim_list)
    axs[1, 0].set_title("RGB SSIM")
    axs[1, 0].set_xlabel("Time Step")
    axs[1, 0].set_ylabel("SSIM")
    axs[1, 1].plot(final_params['keyframe_time_indices'], lpips_list)
    ...
    if swanlab_run is not None:
        swanlab_run.log({"Eval/Metrics": swanlab.Image(fig)})
    plt.close()


def report_loss(losses,
                swanlab_run,
                swanlab_step,
                tracking=False,
                mapping=False):
    ...
    if tracking:
        ...
        tracking_loss_dict['Per Iteration Tracking/step'] = swanlab_step
        swanlab_run.log(tracking_loss_dict)
    elif mapping:
        mapping_loss_dict = {}
        for k, v in loss_dict.items():
            mapping_loss_dict[f"Per Iteration Mapping/{k}"] = v
        mapping_loss_dict['Per Iteration Mapping/step'] = swanlab_step
        swanlab_run.log(mapping_loss_dict)
    else:
        frame_opt_loss_dict = {}
        for k, v in loss_dict.items():
            frame_opt_loss_dict[
                f"Per Iteration Current Frame Optimization/{k}"] = v
        frame_opt_loss_dict[
            'Per Iteration Current Frame Optimization/step'] = swanlab_step
        swanlab_run.log(frame_opt_loss_dict)

    # Increment swanlab step
    swanlab_step += 1
    return swanlab_step
    ...
    if i % every_i == 0 or i == 1:
        ...
        if tracking:
            ...
            if iter_time_idx > 0:
                ...
                rel_gt_w2c = relative_transformation(gt_w2c_list[-2],
                                                     gt_w2c_list[-1])
                rel_est_w2c = relative_transformation(latest_est_w2c_list[-2],
                                                      latest_est_w2c_list[-1])
        ...
        depth_sil, _, _, = Renderer(raster_settings=data['cam'])(
            **depth_sil_rendervar)
        rastered_depth = depth_sil[0, :, :].unsqueeze(0)
        ...
        if tracking:
            psnr = calc_psnr(im * presence_sil_mask,
                             data['im'] * presence_sil_mask).mean()
        else:
        ...
        if not (tracking or mapping):
            progress_bar.set_postfix({
                f"Time-Step: {iter_time_idx} | PSNR: {psnr:.{7}} | Depth L1":
                    f"{rmse:.{7}}"
            })
            progress_bar.update(every_i)
        elif tracking:
            # progress_bar.set_postfix({f"Time-Step: {iter_time_idx} | Rel Pose
            # Error: {rel_pt_error.item():.{7}} | Pose Error:
            # {iter_pt_error.item():.{7}} | ATE RMSE":
            # f"{ate_rmse.item():.{7}}"})
            postfix_key = (f"Time-Step: {iter_time_idx} | Pose Error: "
                           f"{iter_pt_error.item():.{7}} | ATE RMSE")
            progress_bar.set_postfix({postfix_key: f"{ate_rmse.item():.{7}}"})
            progress_bar.update(every_i)
        elif mapping:
            postfix_key = (f"Time-Step: {online_time_idx} | Frame "
                           f"{data['id']} | PSNR: {psnr:.{7}} | Depth L1")
            progress_bar.set_postfix({postfix_key: f"{rmse:.{7}}"})
            progress_bar.update(every_i)
        ...
        if swanlab_run is not None:
            swanlab_log = {
                ...
                f"{stage}/step": swanlab_step
            }
            if tracking:
                swanlab_log = {**swanlab_log, **tracking_log}
            swanlab_run.log(swanlab_log)

        if swanlab_save_qual and (i % qual_every_i == 0 or i == 1):
            # Silhouette Mask
            presence_sil_mask = presence_sil_mask.detach().cpu().numpy()

            # Log plot to swanlab
            if not mapping:
                fig_title = (f"Time-Step: {iter_time_idx} | Iter: {i} | "
                             f"Frame: {data['id']}")
            else:
                fig_title = (f"Time-Step: {online_time_idx} | Iter: {i} | "
                             f"Frame: {data['id']}")
            plot_rgbd_silhouette(data['im'],
                                 data['depth'],
                                 im,
                                 rastered_depth,
                                 presence_sil_mask,
                                 ...
                                 fig_title,
                                 swanlab_run=swanlab_run,
                                 swanlab_step=swanlab_step,
                                 swanlab_title=f"{stage} Qual Viz")


def plot_rgbd_silhouette(color,
                         depth,
                         rastered_color,
                         rastered_depth,
                         presence_sil_mask,
    ...
    # Plot the Ground Truth and Rasterized RGB & Depth, along with Diff Depth
    # & Silhouette
    ...
    # 替换为 SwanLab 记录逻辑
    if swanlab_run is not None:
        if swanlab_step is None:
            swanlab_run.log({swanlab_title: swanlab.Image(fig)})
        else:
            swanlab_run.log({swanlab_title: swanlab.Image(fig)},
                            step=swanlab_step)
...
def evaluate_ate_and_save(gt_traj, est_traj, eval_dir):
    """
    Input :
        gt_traj: list of 4x4 matrices
        est_traj: list of 4x4 matrices
    ...
    np.savetxt(os.path.join(eval_dir, "ate.txt"), trans_error)
...
def align(model, data):
    ...
    for column in range(model.shape[1]):
        W += np.outer(model_zerocentered[:, column], data_zerocentered[:,
                                                                       column])
