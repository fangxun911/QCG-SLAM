...
def _sqrt_positive_part(x: torch.Tensor) -> torch.Tensor:
    ...
    Source: PyTorch3D rotation_conversions.matrix_to_quaternion.
    """
...
def params2rendervar(params):
    rendervar = {
        ...
        'means2D':
            torch.zeros_like(
                params['means3D'], requires_grad=True, device="cuda") + 0
    }
    return rendervar
...
def report_progress(params,
    ...
    if i % every_i == 0 or i == 1:
        ...
        if not mapping:
            postfix_key = (f"Time-Step: {iter_time_idx} | Frame "
                           f"{data['id']} | PSNR: {psnr:.{7}} | RMSE")
...
def eval(dataset,
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        if mapping_iters == 0 and not add_new_gaussians:
            ...
            ssim = ms_ssim(im.unsqueeze(0).cpu(),
    ...
    # fig.suptitle("Average PSNR: {:.2f}, Average Depth RMSE:
    # {:.2f}".format(avg_psnr, avg_rmse), y=1.05, fontsize=16)
