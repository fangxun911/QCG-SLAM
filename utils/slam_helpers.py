...
def params2rendervar(params):
    ...
    rendervar = {
        ...
        'means2D':
            torch.zeros_like(params['means3D'],
                             requires_grad=True,
                             device=params['means3D'].device) + 0
    }
    return rendervar
...
def transformed_params2rendervar(params, transformed_gaussians):
    ...
    rendervar = {
        ...
        'colors_precomp':
            params['rgb_colors'],
        'rotations':
            F.normalize(transformed_gaussians['unnorm_rotations']),
        'opacities':
            torch.sigmoid(params['logit_opacities']),
        'scales':
            torch.exp(log_scales),
...
def get_depth_and_silhouette(pts_3D, w2c):
    ...
    depth_silhouette = torch.zeros((pts_3D.shape[0], 3),
                                   device=depth_z.device).float()
...
def transformed_params2depthplussilhouette(params, w2c, transformed_gaussians):
    ...
    rendervar = {
        'means3D':
            transformed_gaussians['means3D'],
        'colors_precomp':
            get_depth_and_silhouette(transformed_gaussians['means3D'], w2c),
        'rotations':
            F.normalize(transformed_gaussians['unnorm_rotations']),
        'opacities':
            torch.sigmoid(params['logit_opacities']),
...
def transform_to_frame(params, time_idx, gaussians_grad, camera_grad):
    """Transform isotropic or anisotropic Gaussians to the camera frame.
    ...
    Returns:
        transformed_gaussians: Transformed Gaussians as a dict containing
            means3D and unnorm_rotations.
    """
    ...
    rel_w2c = torch.eye(4, device=cam_rot.device).float()
    ...
    pts_ones = torch.ones(pts.shape[0], 1, device=pts.device).float()
