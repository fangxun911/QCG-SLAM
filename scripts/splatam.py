"""Run the baseline SplaTAM RGB-D SLAM pipeline."""
...
setproctitle.setproctitle("original_splatam")


def get_dataset(config_dict, basedir, sequence, **kwargs):
    """Create the dataset object configured for the current sequence."""
...
def calculate_contrast(img_tensor):
    """Calculate the grayscale contrast range for an image tensor."""
    # 计算灰度图像的最大值和最小值
...
def split_image(img, threshold):
    """Split an image into quadtree regions below a contrast threshold."""
    # 分割图像，直到每个区域的对比度小于阈值
    return _split_image(img, threshold, (0, 0, img.shape[3], img.shape[2]))
...
def _split_image(img, threshold, bbox):
    ...
    if contrast < threshold or S_box <= 3:
        return [bbox]
    else:
        # mid_x, mid_y = x + w // 2, y + h // 2
        return (_split_image(img, threshold,
                             (x, y, (w + x) // 2, (h + y) // 2)) +
                _split_image(img, threshold,
                             ((w + x) // 2, y, w, (h + y) // 2)) +
                _split_image(img, threshold,
                             (x, (h + y) // 2, (w + x) // 2, h)) +
                _split_image(img, threshold,
...
def visualize_quadtree(img, quadtree):
    """Draw quadtree boxes on an image."""
...
def get_pointcloud(color,
    ...
    x_grid, y_grid = torch.meshgrid(torch.arange(width).cuda(device).float(),
                                    torch.arange(height).cuda(device).float(),
                                    indexing='xy')
    xx = (x_grid - CX) / FX
    yy = (y_grid - CY) / FY
    xx = xx.reshape(-1)
    ...
    if transform_pts:
        pix_ones = torch.ones(height * width, 1).cuda(device).float()
...
def initialize_params(init_pt_cld, num_frames, mean3_sq_dist,
    ...
    # Initialize a single gaussian trajectory to model the camera poses
    ...
    for k, v in params.items():
        ...
        if not isinstance(v, torch.Tensor):
            params[k] = torch.nn.Parameter(
                torch.tensor(v).cuda(
                    device).float().contiguous().requires_grad_(True))
        else:
            params[k] = torch.nn.Parameter(
                v.cuda(device).float().contiguous().requires_grad_(True))

    variables = {
        'max_2D_radius':
            torch.zeros(params['means3D'].shape[0]).cuda(device).float(),
...
def get_loss(params,
    ...
    torchvision.utils.save_image(im, 'fx/original/rendered_im_%.4f.png' % loss)
    sil_output = torch.where((silhouette > 0.99), silhouette,
                             torch.zeros_like(silhouette))
    torchvision.utils.save_image(sil_output,
                                 'fx/original/rendered_sil_%.4f.png' % loss)
...
def add_new_gaussians(params, variables, curr_data, sil_thres, time_idx,
    ...
    """Add Gaussians for currently unobserved silhouette regions."""
...
def initialize_camera_pose(params, curr_time_idx, forward_prop):
    """Initialize the current camera pose estimate."""
    with torch.no_grad():
...
def rgbd_slam(config: dict):
    """Run the SplaTAM RGB-D tracking and mapping pipeline."""
    # Print Config
    print("Loaded Config:")
    ...
    if config['load_checkpoint']:
        ...
        params = {
            k: torch.tensor(params[k]).cuda(device).float().requires_grad_(True)
            for k in params.keys()
        }
        variables['max_2D_radius'] = torch.zeros(
            params['means3D'].shape[0]).cuda(device).float()
        variables['means2D_gradient_accum'] = torch.zeros(
            params['means3D'].shape[0]).cuda(device).float()
        variables['denom'] = torch.zeros(
            params['means3D'].shape[0]).cuda(device).float()
        variables['timestep'] = torch.zeros(
            params['means3D'].shape[0]).cuda(device).float()
    ...
    for time_idx in tqdm(range(checkpoint_time_idx, num_frames)):
        ...
        if time_idx == 0 or (time_idx + 1) % config['map_every'] == 0:
            ...
            with torch.no_grad():
                ...
                selected_keyframes = keyframe_selection_overlap(
                    depth, curr_w2c, intrinsics, keyframe_list[:-1],
                    num_keyframes)
                selected_time_idx = [
                ...
                print(f"\nSelected Keyframes at Frame {time_idx}: "
                      f"{selected_time_idx}")
            ...
            if num_iters_mapping > 0:
                progress_bar = tqdm(range(num_iters_mapping),
                                    desc=f"Mapping Time Step: {time_idx}")
            for iter in range(num_iters_mapping):
                iter_start_time = time.time()
                # Randomly select a frame until current time step amongst
                # keyframes
        ...
        if ((time_idx == 0) or
            ...
            (time_idx == num_frames - 2)) and (not torch.isinf(
                curr_gt_w2c[-1]).any()) and (not torch.isnan(
                    curr_gt_w2c[-1]).any()):
            with torch.no_grad():
                # Get the current estimated rotation & translation
                curr_cam_rot = F.normalize(
        ...
        # Checkpoint every iteration                 保存checkpoints
        if time_idx % config["checkpoint_interval"] == 0 and config[
                'save_checkpoints']:
            ckpt_output_dir = os.path.join(config["workdir"],
                                           config["run_name"])
            save_params_ckpt(params, ckpt_output_dir, time_idx)
