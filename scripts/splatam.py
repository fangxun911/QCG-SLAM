...
def _split_image(img, threshold, bbox):
    ...
    if contrast < threshold or S_box <= 3:
        ...
        return (_split_image(img, threshold, (x, y, (w + x) // 2, (h + y) // 2)) +
                _split_image(img, threshold, ((w + x) // 2, y, w, (h + y) // 2)) +
                _split_image(img, threshold, (x, (h + y) // 2, (w + x) // 2, h)) +
                _split_image(img, threshold, ((w + x) // 2, (h + y) // 2, w, h)))
...
def initialize_params(init_pt_cld, num_frames, mean3_sq_dist, gaussian_distribution):
    ...
    for k, v in params.items():
        ...
        if not isinstance(v, torch.Tensor):
            params[k] = torch.nn.Parameter(torch.tensor(v).cuda(device).float().contiguous().requires_grad_(True))
        else:
            params[k] = torch.nn.Parameter(v.cuda(device).float().contiguous().requires_grad_(True))

    variables = {'max_2D_radius': torch.zeros(params['means3D'].shape[0]).cuda(device).float(),
                 'means2D_gradient_accum': torch.zeros(params['means3D'].shape[0]).cuda(device).float(),
                 'denom': torch.zeros(params['means3D'].shape[0]).cuda(device).float(),
                 'timestep': torch.zeros(params['means3D'].shape[0]).cuda(device).float()}
...
def get_loss(params, curr_data, variables, iter_time_idx, loss_weights, use_sil_for_loss,
    ...
    sil_output = torch.where((silhouette>0.99), silhouette, torch.zeros_like(silhouette))
    torchvision.utils.save_image(sil_output, 'fx/original/rendered_sil_%.4f.png'%loss)
...
def rgbd_slam(config: dict):
    ...
    if config['load_checkpoint']:
        ...
        params = {k: torch.tensor(params[k]).cuda(device).float().requires_grad_(True) for k in params.keys()}
        variables['max_2D_radius'] = torch.zeros(params['means3D'].shape[0]).cuda(device).float()
        variables['means2D_gradient_accum'] = torch.zeros(params['means3D'].shape[0]).cuda(device).float()
        variables['denom'] = torch.zeros(params['means3D'].shape[0]).cuda(device).float()
        variables['timestep'] = torch.zeros(params['means3D'].shape[0]).cuda(device).float()
