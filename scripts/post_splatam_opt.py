...
from utils.gs_external import (calc_ssim, densify, get_expon_lr_func,
                               update_learning_rate, build_rotation)
...
def initialize_first_timestep_from_ckpt(ckpt_path, dataset, num_frames,
                                        lrs_dict, mean_sq_dist_method):
    ...
    for k in [
            'intrinsics', 'w2c', 'org_width', 'org_height', 'gt_w2c_all_frames',
    ...
    variables['max_2D_radius'] = torch.zeros(
        params['means3D'].shape[0]).cuda().float()
...
def rgbd_slam(config: dict):
    ...
    if config['use_wandb']:
        ...
        wandb_run.define_metric("Number of Gaussians - Densification",
                                step_metric="Mapping_Iters")
        wandb_run.define_metric("Learning Rate - Means3D",
                                step_metric="Mapping_Iters")
    ...
    params, variables, optimizer, intrinsics, w2c, cam = (
        initialize_first_timestep_from_ckpt(ckpt_path, mapping_dataset,
                                            num_frames,
    ...
    for time_idx in tqdm(range(num_frames)):
        ...
        if config['use_wandb']:
            wandb_run.log({
                "Init/Number of Gaussians": post_num_pts,
        ...
        if (time_idx + 1) == num_frames:
            ...
            for iter in range(num_iters_mapping):
                ...
                updated_lr = update_learning_rate(optimizer, means3D_scheduler,
                                                  iter + 1)
                if config['use_wandb']:
                ...
                with torch.no_grad():
                    ...
                    if config['train']['use_gaussian_splatting_densification']:
                        ...
                        if config['use_wandb']:
                            wandb_run.log({
                                "Number of Gaussians - Densification":
                    ...
                    if (iter + 1) == 7000:
                        ...
                        if config['use_wandb']:
                            eval(eval_dataset,
                                 ...
                                 sil_thres=config['train']['sil_thres'],
                                 wandb_run=wandb_run,
                                 wandb_save_qual=config['wandb']
                                 ['eval_save_qual'],
                                 mapping_iters=config["train"]
                                 ["num_iters_mapping"],
