"""Capture a dataset from the NeRFCapture iOS app.

Code is adapted from instant-ngp/scripts/nerfcapture2nerf.py.
...
def dataset_capture_loop(reader: DataReader, save_path: Path, overwrite: bool,
                         n_frames: int, depth_scale: float):
    if save_path.exists():
        if overwrite:
...
if __name__ == "__main__":
    ...
    dataset_capture_loop(reader, Path(config['workdir']), config['overwrite'],
                         config['num_frames'], config['depth_scale'])
